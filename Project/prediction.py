...
if st.sidebar.button("🤖 ML Risk Engine Logs", use_container_width=True, type="primary"):
...
try:
    ...
    df = pd.read_sql_query("""
        SELECT 
            p.prediction_id AS [Prediction ID], 
            r.title AS [Requirement Title], 
            p.predicted_risk_level AS [Predicted Risk Level], 
            p.confidence_score AS [Confidence Score], 
            p.xai_explanation AS [Explainable AI Log],
            p.predicted_at AS [Evaluation Timestamp]
        FROM Predictions p
    ...
    if df.empty:
        st.info("ℹ️ No model evaluation metrics found yet. Analyze a requirement from the dashboard to run the ML engine.")
    else:
        # Formatted confidence presentation
        df['Confidence Score'] = df['Confidence Score'].apply(lambda x: f"{x * 100:.2f}%")
