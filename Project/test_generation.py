...
st.set_page_config(
    page_title="Automated Test Suite",
    page_icon="🧪",
    layout="wide",
...
if st.sidebar.button("🧪 Automated Test Suite", use_container_width=True, type="primary"):
...
try:
    ...
    df = pd.read_sql_query("""
        SELECT 
            tc_id AS [Test Case ID],
            test_scenario AS [Scenario Context],
            test_objective AS [Objective Goals],
            test_steps AS [Step Vectors],
            expected_result AS [Expected Bounds],
            test_case_type AS [Type Profile],
            created_at AS [Generation Date]
        FROM GeneratedTestCases
    """, conn)
    conn.close()
    
    if df.empty:
        st.info("ℹ️ Test execution profiles are currently empty. Run a pipeline verification pass to populate cases.")
    else:
